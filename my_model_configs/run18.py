...
def make_model(shape):
        s = Input(shape=shape) 
        x = Conv1D(n_kernels,kernel_size_1,activation='relu',padding='same')(s)
        x = Conv1D(n_kernels,kernel_size_1,activation='relu',padding='same')(x)
        x = BatchNormalization()(x)
        x = squeeze_excite_block(n_kernels,x)
        x = AveragePooling1D(pool_size_param)(x)

        x = Conv1D(2*n_kernels,kernel_size_2,activation='relu',padding='same')(x)
        x = Conv1D(2*n_kernels,kernel_size_2,activation='relu',padding='same')(x)
        x = BatchNormalization()(x)
        x = squeeze_excite_block(2*n_kernels,x)
        x = AveragePooling1D(pool_size_param)(x)
        x = Dropout(dropout_percent)(x)

        x = Conv1D(4*n_kernels,kernel_size_3,activation='relu',padding='same')(x)
        x = Conv1D(4*n_kernels,kernel_size_3,activation='relu',padding='same')(x)
        x = BatchNormalization()(x)
        x = squeeze_excite_block(4*n_kernels,x)
        x = AveragePooling1D(pool_size_param)(x)
        x = Dropout(dropout_percent)(x)
