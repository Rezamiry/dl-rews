...
model = Sequential()
model.add(ConvLSTM2D(filters=128, dropout=dropout_percent, recurrent_dropout=dropout_percent, kernel_size=(1,5), activation='relu',return_sequences=True, input_shape=(n_steps, 1, n_length, n_features)))
model.add(ConvLSTM2D(filters=64, dropout=dropout_percent, recurrent_dropout=dropout_percent, kernel_size=(1,3), activation='relu'))
model.add(Flatten())
