...
x = Conv1D(filters=n_kernels, kernel_size=kernel_size_1, kernel_initializer = initializer_param, padding='same')(input_x)
x = Conv1D(filters=n_kernels, kernel_size=kernel_size_1, kernel_initializer = initializer_param, padding='same')(input_x)
x = Activation('relu')(x)
x = MaxPooling1D(pool_size=pool_size_param)(x)
x = Dropout(dropout_percent)(x)

x = Conv1D(filters=2*n_kernels,  kernel_size=kernel_size_2,  kernel_initializer = initializer_param, padding='same')(x)
x = Conv1D(filters=2*n_kernels,  kernel_size=kernel_size_2,  kernel_initializer = initializer_param, padding='same')(x)
x = Activation('relu')(x)
x = MaxPooling1D(pool_size=pool_size_param)(x)
x = Dropout(dropout_percent)(x)

x = Conv1D(filters=4*n_kernels,  kernel_size=kernel_size_3,  kernel_initializer = initializer_param, padding='same')(x)
x = Conv1D(filters=4*n_kernels,  kernel_size=kernel_size_3,  kernel_initializer = initializer_param, padding='same')(x)
...
es = EarlyStopping(monitor='val_accuracy', mode='max', verbose=1, patience=400)
