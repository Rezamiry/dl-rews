...
x = Conv1D(filters=n_kernels, strides=stride_size, kernel_size=kernel_size,  padding='same')(input_x)
x = BatchNormalization()(x)
x = Activation('relu')(x)
x = MaxPooling1D(pool_size=pool_size_param)(x)
x = Dropout(dropout_percent)(x)

x = Conv1D(filters=2*n_kernels,  kernel_size=kernel_size,  padding='same')(x)
x = BatchNormalization()(x)
x = Activation('relu')(x)
x = MaxPooling1D(pool_size=pool_size_param)(x)
x = Dropout(dropout_percent)(x)
...
x = Conv1D(filters=4*n_kernels,  kernel_size=kernel_size,  padding='same')(x)
x = BatchNormalization()(x)
x = Activation('relu')(x)
x = MaxPooling1D(pool_size=pool_size_param)(x)
x = Dropout(dropout_percent)(x)
